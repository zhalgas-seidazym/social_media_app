...
class PostListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        if filter == 'saved':
            ...
            return sorted(Post.objects.all(), key=popularity_score, reverse=True)
        else:
            # Fetch posts in three stages: followings, others, user
            following_users = user.following.all()  # Assuming you have a ManyToMany field named 'following' on User model
            following_posts = Post.objects.filter(creator__in=following_users).order_by("-created")
            other_posts = Post.objects.exclude(creator=user).exclude(creator__in=following_users).order_by("-created")
            user_posts = user.posts.all().order_by("-created")

            # Combine the querysets maintaining the order
            combined_posts = list(following_posts) + list(other_posts) + list(user_posts)
